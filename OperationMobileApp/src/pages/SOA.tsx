...
import { Search, Filter, FileText, ArrowLeft, RefreshCw, X, CreditCard, ExternalLink } from 'lucide-react-native';
...
const SOA: React.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (authData) {
        ...
        // In web, username was account_no. Assuming same here.
  ...
  const fetchRecords = async (isInitial = false) => {
    try {
      ...
      let records: SOARecord[] = [];

      if (role === 'customer') {
        // Usually customer viewing their own SOA uses getStatementsByAccount or a similar filtered endpoint
        // Ideally the backend filters by the logged-in user if calling getAll but let's assume filtering happens there
        // However, the web code used `useSOAContext` which fetched ALL records.
        // If migration is 1:1, we fetch all (maybe filtered by API automatically).
        // BUT for customer app, we should probably fetch specific to account.
        // Assuming `soaService` might need adjustment or API handles it.
        // Let's rely on standard fetch.
        records = await soaService.getAllStatements(true);
        // Optimization: Filter locally if API returns everything (bad practice but matching context behavior)
        if (user.username) {
      ...
      records.forEach(r => {
        if (r.statement_date) dates.add(r.statement_date);
      });
  ...
  // Filter
  const filteredRecords = soaRecords.filter(r => {
    const matchDate = selectedDate === 'All' || r.statement_date === selectedDate;
  ...
  const renderItem = ({ item }: { item: SOARecord }) => (
    <TouchableOpacity
      ...
      <View className="flex-row justify-between items-start">
        ...
        <View className="items-end">
          <Text className={`font-bold ${colorPalette?.primary ? '' : 'text-orange-600'}`} style={colorPalette?.primary ? { color: colorPalette.primary } : {}}>
            ₱{item.total_amount_due ? item.total_amount_due.toFixed(2) : '0.00'}
          </Text>
  ...
  if (selectedRecord) {
    return (
      <SafeAreaView className={`flex-1 ${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
        <SOADetails soaRecord={selectedRecord} onClose={() => setSelectedRecord(null)} />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className={`flex-1 ${isDarkMode ? 'bg-gray-950' : 'bg-gray-50'}`}>
      ...
      <View className={`border-b px-4 py-3 ${isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
        <View className="flex-row justify-between items-center mb-4">
          <Text className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Statements (SOA)</Text>
