...
const PaymentMethodList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      loadPaymentMethods(true).catch((err) => console.error('Idle refresh failed:', err));
    }, IDLE_TIME_LIMIT);
  ...
  const executeDelete = async (paymentMethod: PaymentMethod) => {
    ...
    setDeletingItems((prev) => new Set(prev).add(paymentMethod.id));
    showGlobalModal('loading', 'Deleting', `Removing payment method "${paymentMethod.payment_method}"...`);

    try {
      const response = await apiClient.delete(`/payment-methods/${paymentMethod.id}`);
      const data = response.data;
  ...
  const filteredPaymentMethods = paymentMethods.filter((pm) => {
    const matchesOrg = currentUserOrgId ? pm.organization_id === currentUserOrgId : !pm.organization_id;
  ...
  const renderListItem = ({ item: pm }: { item: PaymentMethod }) => (
    <TouchableOpacity
      onPress={() => handleEdit(pm)}
      activeOpacity={0.7}
      style={{
      ...
      <View style={{ flex: 1, minWidth: 0, paddingRight: 16 }}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 12, marginTop: 4 }}>
          <Text style={{ fontSize: 10, textTransform: 'uppercase', fontWeight: '500', color: '#9ca3af' }}>
            Created: {formatDate(pm.created_at)}
          </Text>
          <Text style={{ fontSize: 10, textTransform: 'uppercase', fontWeight: '500', color: '#9ca3af' }}>
            Updated: {formatDate(pm.updated_at)}
          </Text>
        </View>
      </View>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
        <TouchableOpacity onPress={() => handleEdit(pm)} style={{ padding: 8, borderRadius: 6 }}>
          <Edit2 size={18} color="#4b5563" />
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => handleDelete(pm)}
          disabled={deletingItems.has(pm.id)}
          style={{ padding: 8, borderRadius: 6, opacity: deletingItems.has(pm.id) ? 0.5 : 1 }}
        >
    ...
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      ...
      >
        ...
        >
          ...
          {isTablet && <Text style={{ color: '#ffffff', fontSize: 12, fontWeight: '500' }}>Add Method</Text>}
        </TouchableOpacity>
        <TouchableOpacity
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={{ paddingVertical: 80, alignItems: 'center' }}>
              <Text style={{ color: '#6b7280' }}>No payment methods found</Text>
            </View>
          }
        />
      )}
