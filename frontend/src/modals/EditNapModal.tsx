...
const EditNapModal: React.FC<EditNapModalProps> = ({ isOpen, onClose, onSave, napItem }) => {
  const [formData, setFormData] = useState<NapFormData>({ name: '' });
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (isOpen) {
        setFormData({ name: napItem ? napItem.nap_name : '' });
  ...
  const handleSave = async () => {
    if (!validateForm()) {
      setModal({ isOpen: true, type: 'warning', title: 'Validation Error', message: 'NAP Name is required.' });
      return;
    }
  ...
  return (
    ...
    >
      <EditNapContent formData={formData} setFormData={setFormData} errors={errors} modifiedDate={modifiedDate} modifiedBy={modifiedBy} />
    </ModalUITemplate>
  );
};
    ...
    <View style={{ gap: 16 }}>
      <View style={{ gap: 8 }}>
        <Text style={{ fontSize: 14, fontWeight: '500', color: labelColor }}>
          NAP Name<Text style={{ color: '#ef4444' }}> *</Text>
        </Text>
        <TextInput
          value={formData.name}
