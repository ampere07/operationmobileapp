...
export const formatToGMT8MySQL = (date: Date = new Date()): string => {
  return dayjs(date).tz(MANILA_TIMEZONE).format('YYYY-MM-DD HH:mm:ss');
};
...
export const formatToGMT8Display = (date: Date = new Date()): string => {
  return dayjs(date).tz(MANILA_TIMEZONE).format('MM/DD/YYYY, hh:mm:ss A');
};
...
export const getGMT8DateOnly = (date: Date = new Date()): string => {
  return dayjs(date).tz(MANILA_TIMEZONE).format('YYYY-MM-DD');
};
